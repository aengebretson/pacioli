...
Provenance provenance(const char* source) {
  auto value = Provenance::create(
      {SourceRecordId{source}}, "lifecycle-projection.fixture", "1");
  assert(value);
  return *value;
}

EventHeader header_for(const char* record_id, Timestamp effective_at,
                       const char* source) {
  auto value = EventHeader::create(EventId{record_id}, AccountId{"fund-a"},
                                   effective_at, provenance(source));
  assert(value);
  return *value;
}

EconomicEvent cash(const char* record_id, Timestamp effective_at,
...
EconomicEvent cash_scaled(const char* record_id, Timestamp effective_at,
                          std::int64_t amount, const char* source) {
  return CashMovement::create(header_for(record_id, effective_at, source),
                              Money::from_scaled(amount, usd));
}

EconomicEvent trade(const char* record_id, Timestamp effective_at,
                    const char* quantity, const char* price,
                    year_month_day settlement_date, const char* source) {
  ...
  auto value = EquityTrade::create(
      header_for(record_id, effective_at, source), InstrumentId{"MSFT"},
...
void accept(LifecycleLedger& ledger, const LifecycleRecordDraft& draft) {
  if (!ledger.accept(draft)) std::abort();
}

LifecycleProjectionResult project(const LifecycleResolution& resolution,
...
void expect_no_obligations(const LifecycleProjectionResult& result) {
  assert(result.open_settlement_obligations &&
         result.open_settlement_obligations->empty());
}

void expect_position(const LifecycleProjectionResult& result,
                     const char* quantity) {
  assert(result.positions && result.positions->size() == 1);
  assert((result.positions->front().key() ==
          PositionKey{AccountId{"fund-a"}, InstrumentId{"MSFT"}}));
...
void expect_cash(const LifecycleProjectionResult& result, const char* amount) {
  ...
  assert((result.settled_cash->front().key() ==
          CashKey{AccountId{"fund-a"}, usd}));
  assert(result.settled_cash->front().amount() == *Money::parse(amount, usd));
}

void expect_obligation(const LifecycleProjectionResult& result,
                       year_month_day settlement_date,
                       SettlementDirection direction, const char* amount) {
  assert(result.open_settlement_obligations &&
         result.open_settlement_obligations->size() == 1);
  const auto& obligation = result.open_settlement_obligations->front();
  assert((obligation.key() == SettlementObligationKey{
                                  AccountId{"fund-a"},
...
void late_cash_correction_and_cancellation() {
  ...
  accept(ledger, LifecycleRecordDraft::originate(
                     ...
                     cash("cash-record-v1", effective_at, "1000",
                          "cash-source-original")));
  accept(ledger, LifecycleRecordDraft::correct(
                     EconomicEventId{"cash-economic-1"},
                     EventId{"cash-record-v1"}, recorded_correction,
                     cash("cash-record-v2", effective_at, "1200",
                          "cash-source-correction")));
  accept(ledger, LifecycleRecordDraft::cancel(
                     EventId{"cash-record-v3"},
                     EconomicEventId{"cash-economic-1"},
                     EventId{"cash-record-v2"}, AccountId{"fund-a"},
                     recorded_cancellation,
                     provenance("cash-source-cancellation")));

  const auto before_resolution =
      ledger.resolve(recorded_correction - 1ns, economic_as_of);
  ...
  const auto corrected_resolution =
      ledger.resolve(recorded_correction, economic_as_of);
  const auto corrected =
      project(corrected_resolution, economic_as_of, date(1, 31));
  ...
  assert(corrected_event.lineage()[0].get().provenance() ==
         provenance("cash-source-original"));
  assert(corrected_event.lineage()[1].get().provenance() ==
         provenance("cash-source-correction"));

  const auto cancelled_resolution =
      ledger.resolve(recorded_cancellation, economic_as_of);
  const auto cancelled =
      project(cancelled_resolution, economic_as_of, date(1, 31));
  ...
  assert(cancelled_resolution.chains().front().head().record_id() ==
         EventId{"cash-record-v3"});
  ...
  assert(ledger.records()[2].provenance() ==
         provenance("cash-source-cancellation"));
}
...
void equity_correction_and_reversal() {
  ...
  accept(ledger, LifecycleRecordDraft::originate(
                     ...
                     cash("opening-cash-record", opening_effective, "100000",
                          "opening-cash-source")));
  accept(ledger, LifecycleRecordDraft::originate(
  ...
  accept(ledger, LifecycleRecordDraft::reverse(
                     EconomicEventId{"trade-reversal-economic-1"},
                     EventId{"trade-record-v2"}, recorded_reversal,
                     trade("reversal-record-v1", reversal_effective, "-80",
                           "55", date(6, 6), "trade-source-reversal")));

  const auto original_resolution =
      ledger.resolve(recorded_trade, trade_date_as_of);
  const auto original =
      project(original_resolution, trade_date_as_of, date(6, 2));
  expect_position(original, "100");
  expect_cash(original, "100000");
  expect_obligation(original, date(6, 4), SettlementDirection::payable,
                    "5000");

  const auto corrected_resolution =
      ledger.resolve(recorded_correction, trade_date_as_of);
  const auto corrected =
      project(corrected_resolution, trade_date_as_of, date(6, 2));
  expect_position(corrected, "80");
  expect_cash(corrected, "100000");
  expect_obligation(corrected, date(6, 4), SettlementDirection::payable,
                    "4400");
  ...
  const auto settled = project(settled_resolution, corrected_settlement_as_of,
                               date(6, 4));
  expect_position(settled, "80");
  expect_cash(settled, "95600");
  expect_no_obligations(settled);

  const auto reversed_resolution =
      ledger.resolve(recorded_reversal, reversal_as_of);
  const auto reversed =
      project(reversed_resolution, reversal_as_of, date(6, 5));
  expect_no_positions(reversed);
  expect_cash(reversed, "95600");
  expect_obligation(reversed, date(6, 6),
                    SettlementDirection::receivable, "4400");
  ...
  const auto final_resolution =
      ledger.resolve(recorded_reversal, reversal_settlement_as_of);
  const auto final = project(final_resolution, reversal_settlement_as_of,
                             date(6, 6));
  ...
  assert(std::get<EquityTrade>(*ledger.records()[2].event()).quantity() ==
         *Quantity::parse("80"));
}
...
void deterministic_across_acceptance_paths() {
  ...
  const auto individual_resolution =
      individually_accepted.resolve(recorded_at, effective_at);
  const auto batch_resolution =
      batch_accepted.resolve(recorded_at, effective_at);
  const auto individual =
      project(individual_resolution, effective_at, date(1, 1));
  ...
  assert(individual.open_settlement_obligations ==
         batch.open_settlement_obligations);
  assert(individual.positions == repeated.positions);
  assert(individual.settled_cash == repeated.settled_cash);
  assert(individual.open_settlement_obligations ==
         repeated.open_settlement_obligations);
...
void projection_errors_remain_exact() {
  ...
  accept(cash_overflow, LifecycleRecordDraft::originate(
                            EconomicEventId{"cash-max-economic"}, recorded_at,
                            cash_scaled("cash-max", effective_at,
                                        std::numeric_limits<std::int64_t>::max(),
                                        "cash-max-source")));
  accept(cash_overflow, LifecycleRecordDraft::originate(
                            EconomicEventId{"cash-plus-economic"}, recorded_at,
                            cash_scaled("cash-plus", effective_at, 1,
                                        "cash-plus-source")));
  accept(cash_overflow, LifecycleRecordDraft::originate(
                            EconomicEventId{"cash-minus-economic"}, recorded_at,
                            cash_scaled("cash-minus", effective_at, -1,
                                        "cash-minus-source")));
  const auto cash_resolution =
      cash_overflow.resolve(recorded_at, effective_at);
  const auto cash_result =
      project(cash_resolution, effective_at, date(1, 1));
  assert(!cash_result.settled_cash);
  assert(cash_result.settled_cash.error() ==
         CashProjectionError::amount_overflow);
  ...
  accept(settlement_overflow, LifecycleRecordDraft::originate(
                                  EconomicEventId{"settlement-max-economic"},
                                  recorded_at,
                                  trade("settlement-max", effective_at,
                                        "92233720368.54775807", "100",
                                        date(6, 30), "settlement-max-source")));
  accept(settlement_overflow, LifecycleRecordDraft::originate(
                                  EconomicEventId{"settlement-one-economic"},
                                  recorded_at,
                                  trade("settlement-one", effective_at,
                                        "0.000001", "1", date(6, 30),
                                        "settlement-one-source")));
  const auto settlement_resolution =
      settlement_overflow.resolve(recorded_at, effective_at);
  const auto settlement_result =
      project(settlement_resolution, effective_at, date(6, 1));
  assert(!settlement_result.positions);
  assert(settlement_result.positions.error() ==
         PositionProjectionError::quantity_overflow);
  assert(settlement_result.settled_cash &&
         settlement_result.settled_cash->empty());
  ...
  accept(valuation_overflow, LifecycleRecordDraft::originate(
                                 EconomicEventId{"settled-huge-economic"},
                                 recorded_at,
                                 trade("settled-huge", effective_at,
                                       "92233720368.54775807",
                                       "92233720368.54775807", date(6, 1),
                                       "settled-huge-source")));
  accept(valuation_overflow, LifecycleRecordDraft::originate(
                                 EconomicEventId{"open-huge-economic"},
                                 recorded_at,
                                 trade("open-huge", effective_at,
                                       "-92233720368.54775807",
                                       "92233720368.54775807", date(6, 30),
                                       "open-huge-source")));
  const auto valuation_resolution =
      valuation_overflow.resolve(recorded_at, effective_at);
  const auto valuation_result =
      project(valuation_resolution, effective_at, date(6, 1));
  expect_no_positions(valuation_result);
  assert(!valuation_result.settled_cash);
  assert(valuation_result.settled_cash.error() ==
         CashProjectionError::valuation_overflow);
...
int main() {
  static_assert(std::is_same_v<
                decltype(LifecycleProjectionResult::positions),
                std::expected<std::vector<Position>, PositionProjectionError>>);
  static_assert(std::is_same_v<
                decltype(LifecycleProjectionResult::settled_cash),
                std::expected<std::vector<CashBalance>, CashProjectionError>>);
  static_assert(std::is_same_v<
                decltype(LifecycleProjectionResult::open_settlement_obligations),
                std::expected<std::vector<SettlementObligation>,

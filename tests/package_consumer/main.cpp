...
int main() {
  ...
  const auto provenance = luca::Provenance::create(
      std::vector{luca::SourceRecordId{"consumer-source-1"}}, "package-consumer", "1");
  if (!provenance)
    return 1;
  constexpr luca::Timestamp effective_at{1s};
  const auto header = luca::EventHeader::create(
      luca::EventId{"deposit-1"}, luca::AccountId{"account-1"}, effective_at, *provenance);
  if (!header)
    return 1;
  ...
  const auto accepted_prefix = lifecycle.accept(luca::LifecycleRecordDraft::originate(
      luca::EconomicEventId{"consumer-deposit-economic"}, effective_at,
  ...
  if (!checkpoint_digest || !input_digest)
    return 6;
  const auto projection = luca::CheckpointIdentity::create("consumer.portfolio-state", "1");
  const auto policy = luca::CheckpointIdentity::create("consumer.default-policy", "1");
  const auto partition =
      luca::AccountSetPartition::create(std::vector{luca::AccountId{"account-1"}});
  const auto prefix =
      luca::CheckpointEventPrefix::create(1, 1, 1, luca::EventId{"deposit-1"}, *input_digest);
  ...
  const auto watermark =
      luca::ResolvedEventWatermark::create(effective_at, 1, luca::EventId{"deposit-1"});
  const auto lineage = luca::CheckpointLineage::create(
      std::vector{luca::EventId{"deposit-1"}}, std::vector{luca::EventId{"deposit-1"}},
      std::vector{luca::SourceRecordId{"consumer-source-1"}});
  if (!context || !watermark || !lineage) {
    return 6;
  }
  const auto manifest =
      luca::CheckpointManifest::create(*projection, "consumer-engine-1", *policy, *partition,
                                       *prefix, *context, *checkpoint_digest, *watermark, *lineage);
  if (!manifest)
    return 6;
  ...
  const auto resume = luca::CheckpointResumeRequest::create(
      ...
      "consumer-engine-1", *policy, *partition, *context, *prefix, *checkpoint_digest);
  ...
  const auto suffix_header = luca::EventHeader::create(
      luca::EventId{"deposit-2"}, luca::AccountId{"account-1"}, resume_through, *provenance);
  ...
  if (!suffix_header || !suffix_amount ||
      !lifecycle
           .accept(luca::LifecycleRecordDraft::originate(
               luca::EconomicEventId{"consumer-deposit-2-economic"}, resume_through,
